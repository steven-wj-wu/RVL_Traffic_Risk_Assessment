...
process = Qt()
fileUrl,file_name = process.mv_Chooser()
print(file_name)
if(fileUrl == ""):
...
txt_path = '../Dataset/Long_Drive/label_new/'+file_name
f=open(txt_path,'r')
time_show = []
level_show = []
...
for idx,line in enumerate(f.readlines()):
    data = line.split(',')
    risk_tmp=data[1]
    risk_tmp = risk_tmp.strip()
    time_show.append(idx)
    if str(risk_tmp) == 'normal':
        level_show.append(risk_tmp)
    elif risk_tmp=='dangerous':
        level_show.append('danger')
...
#checkpoint_flow = torch.load('./best_flow_10.pth')
checkpoint_rgb = torch.load('./best_rgb.pth')
...
while(ret):
    ...
    if flag==0:
        ...
        rgb_object_image[weight_frame>50]= resize_frame[weight_frame>50]
        op_object_image[weight_frame>50] = optical_frame[weight_frame>50]
        ...
        if len(full_rgb_clip) > 19:
            ...
            rgb_distract_output = predict(model_rgb, test_data_rgb)
            print('rrr',rgb_distract_output)
            op_distract_output = predict(model_flow, test_data_op)
            ...
            risk_score = (rgb_distract_output+op_distract_output)/2
            risk_score = risk_score.cpu().detach().numpy()
            print(risk_score)
            risk = classes[risk_score.argmax()]

            if risk == classes[0] or risk == classes[1]:
                dn_levlel = 'danger'

            elif risk == classes[2] or risk == classes[3]:
                dn_levlel = 'normal'
        ...
        plt.plot(time_frame,level_show[0:n],color=(1,0,0),label="ground truth")
        plt.plot(time_frame,dn,color=(0,1,0),label="predict")
        ...
        cv2.imshow("frame",show_output)
        cv2.imshow("frame2",rgb_object_image)
...
"""
plt.plot(time,critical_score,color=(1,0,0),label="critical")
plt.plot(time,high_score,color=(0,1,0),label="high")
plt.plot(time,medium_score,color=(1,1,0),label="medium")
plt.plot(time,low_score,color=(0,0,1),label="low")
