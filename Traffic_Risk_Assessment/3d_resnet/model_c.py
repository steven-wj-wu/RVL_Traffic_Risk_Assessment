...
def generate_model(opt):
    assert opt.model in [
        'resnet', 'preresnet', 'wideresnet', 'resnext', 'densenet', 'slowfast','cnnlstm','cnnlstm_flow','resnet_lstm','cnnlstm_alexnet'
    ]
    optimizer = None
    scheduler = None
    ...
    if opt.model == 'resnet':
        ...
        from resnet_models.resnet_lgdv2 import get_fine_tuning_parameters
    ...
    elif opt.model == 'cnnlstm':
        print("use cnnlstm")
        model = cnnlstm.CNNLSTM(num_classes=opt.n_classes)

    elif opt.model == 'cnnlstm_flow':
        print("use cnnlstm_flow")
        model = cnnlstm_flow.CNNLSTM(num_classes=opt.n_classes)

    elif opt.model == 'cnnlstm_alexnet':
        print("use alexnet")
        model = cnnlstm_alexnet.CNNLSTM_ALEXNET(num_classes=opt.n_classes)
