...
def do_opticalflow(prev_frame,next_frame):
	...
	#flow = cv2.calcOpticalFlowFarneback(prev_gray,next_gray, None, 0.5, 3, 25, 3, 7, 1.5, 0) #calc op
	mag, ang = cv2.cartToPolar(flow[...,0], flow[...,1])
	hsv[...,0] = ang*180/np.pi/2
	hsv[...,2] = cv2.normalize(mag,None,0,255,cv2.NORM_MINMAX)
	bgr = cv2.cvtColor(hsv,cv2.COLOR_HSV2BGR)
...
if __name__ == '__main__':
	...
	read_flow_baseUrl ="/media/leo/C0EC4D32EC4D244E/TRA_split_20_ori/flow/validation"
	save_baseUrl = "/media/leo/C0EC4D32EC4D244E/TRA_split_20_ori/weight_flow_224/validation"
	...
	for idx,label in enumerate(TRA_Labels):
		...
		flow_label_url = os.path.join(read_flow_baseUrl,label)
		print(label_url)
		...
		for num,clip in enumerate(clip_list):
			...
			flow_frame_url =  os.path.join(flow_label_url,clip)
			...
			for f_num,frame in enumerate(frame_list):
				if not (frame == 'n_frames'):
					...
					#yolo_detection
					if f_num>0:
						current_yolo_img = frame.copy()
						current_yolo_img=cv2.resize(current_yolo_img,imgSize)
						current_yolo_img = current_yolo_img[..., ::-1]
						current_results = yolo_model(current_yolo_img, size=224)
						current_results = current_results.pandas().xyxy[0]


						flow_frame_path=os.path.join(flow_frame_url,frame_list[f_num-1])
						flow_frame = cv2.imread(flow_frame_path)
						flow_frame = cv2.resize(flow_frame,imgSize)
						blurred = cv2.multiply(flow_frame,(0.4,0.4,0.4,0.4))

						#flow_img = do_opticalflow(prev_frame,frame)
						weight_frame = do_saliency(prev_frame,frame,attention_model)

						for index,row in pre_results.iterrows():
							y =  int(row["ymin"])
							h =  int(row["ymax"])-int(row["ymin"])
							x = int(row["xmin"])
							w = int(row["xmax"])-int(row["xmin"])
							roi = flow_frame[y:y+h,x:x+w]
						...
						blurred[weight_frame>50]=flow_frame[weight_frame>50]
						series.append(blurred)

						prev_frame = frame.copy()
						pre_results = current_results

					if f_num==0:
						pre_yolo_img = frame.copy()
						pre_yolo_img=cv2.resize(pre_yolo_img,imgSize)
						pre_yolo_img =  pre_yolo_img[..., ::-1]
						pre_results = yolo_model(pre_yolo_img, size=224)
						pre_results = pre_results.pandas().xyxy[0]
						prev_frame = frame.copy()
					#print(len(series))
