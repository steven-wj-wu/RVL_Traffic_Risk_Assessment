...
def main():
    ...
    best = torch.load('./checkpoints/attention_op_fre.tar')  #my model
    #best = torch.load('./op.tar')  #my model
    #best = torch.load('./ckpts/cdnn.tar')  #cdnn model

    model.load_state_dict(best['state_dict'])

    cap = cv2.VideoCapture('./video/001.avi')
    fourcc = cv2.VideoWriter_fourcc(*'MP4V')
    out = cv2.VideoWriter('test_video_op.mp4', fourcc, 30, (320,192))
    ...
    while (cap.isOpened()):
        ...
        if ret == True:
            if c%f==0:
                ...
                if flag == 0:
                    prev_frame =  cv2.resize(video_frame, (320, 192), interpolation=cv2.INTER_CUBIC)
                    prev_gray = cv2.cvtColor(prev_frame,cv2.COLOR_BGR2GRAY)
                
                #video_frame = cv2.resize(video_frame, (320, 192), interpolation=cv2.INTER_CUBIC)
                next_frame =  cv2.resize(video_frame, (320, 192), interpolation=cv2.INTER_CUBIC)
